...
if (/mongo/.test(config.options.database)) {
    Database = class MongoDB {
        constructor(url) {
            ...
            this.connection = this.url || config.options.database
            this.model = {
                database: {},
            }
            this.data = {}
...
} else if (/json/.test(config.options.database)) {
    Database = class Database {
        data = {}
        file = path.join(process.cwd(), "lib/data", config.options.database)
