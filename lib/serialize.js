...
sendFile: {
   async value(jid, url, filename, caption = "", quoted = "", options = {}) {
      ...
      if (size > 45000000) data = { document: buffer, mimetype: mime, fileName: filename ? `${filename}.${ext}` : `${conn.user?.name} (${new Date()}).${ext}`, ...options }
      ...
      m.reply = async (text, options = {}) => {
         ...
         if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            ...
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               ...
               return conn.sendMessage(chatId, { text, mentionedJid: [m.sender, ...conn.parseMention(text)], ...options }, { quoted, ephemeralExpiration: m.expiration, ...options })
            } else {
               return conn.sendFile(m.from, data.data, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else if (typeof text == "object") {
            return conn.sendMessage(chatId, { text: func.format(text), mentionedJid: [m.sender, ...conn.parseMention(func.Format(text))], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
         } else {
            if (!!config.msg[text]) text = config.msg[text]
            return conn.sendMessage(chatId, { text, mentionedJid: [m.sender, ...conn.parseMention(text)], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
         }
