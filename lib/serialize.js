      ...
      m.reply = async (text, options = {}) => {
         ...
         if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            ...
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               ...
               return conn.sendFile(m.from, data.data, fileName, caption, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else if (typeof text == "object") {
           return conn.sendMessage(chatId, { text: func.format(text), mentions: [m.sender, ...conn.parseMention(func.Format(text))], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
         } else {
            if (!!config.msg[text]) text = config.msg[text]
