      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            return conn.sendMessage(jid, {
               ...
               mentionedJid: quoted?.participant ? [conn.decodeJid(quoted?.participant)] : [conn.decodeJid(conn?.user?.id)],
               ...options
            }, { quoted, ...options })
         },
         enumerable: true
...
sendFile: {
   ...
   async value(jid, url, fileName = "", caption = "", quoted = "", options = {}) {
      let { mime, res: buffer, ext, size } = await func.getFiles(url, true)
      ...
      if (size > 40000000) data = { document: buffer, mimetype: mime, fileName: fileName ? `${fileName}.${ext}` : `${conn.user?.name} (${new Date()}).${ext}`, ...options }
         ...
         externalAdReply: {
            ...
            thumbnailUrl: fs.readFileSync("./src/qrbe.jpg"),
            sourceUrl: "https://chat.whatsapp.com/JKdIWr5fj990Ux2oqTjTo5",
         }
      }}
      ...
      else if (/image/.test(mime)) data = { image: buffer, fileName: fileName ? `${fileName}.${ext}` : `${conn.user?.name} (${new Date()}).${ext}`, caption : typeof caption == "object" ? Function.format(caption) : caption, mimetype: options?.mimetype ? options.mimetype : "image/png", ...options }
      else if (/video/.test(mime)) data = { video: buffer, fileName: fileName ? `${fileName}.${ext}` : `${conn.user?.name} (${new Date()}).${ext}`, caption : typeof caption == "object" ? Function.format(caption) : caption, mimetype: options?.mimetype ? options.mimetype : "video/mp4", ...options }
      else if (/audio/.test(mime)) data = { audio: buffer, fileName: fileName ? `${fileName}.${ext}` : `${conn.user?.name} (${new Date()}).${ext}`, caption : typeof caption == "object" ? Function.format(caption) : caption, mimetype: options?.mimetype ? options.mimetype : "audio/mpeg", ...options }
         ...
         value(jid, copy, text = "", sender = conn.user.id, options = {}) {
            ...
            if (typeof content !== "string") {
               ...
               copy.message[mtype].contextInfo = {
                  ...(content.contextInfo || {}),
                  mentionedJid: options.mentionedJid || content.contextInfo?.mentionedJid || []
               }
            }
   ...
   if (m.message) {
      ...
      m.reply = async (text, options = {}) => {
         ...
         if (typeof text == "object") {
            return conn.sendMessage(chatId, { text: Function.format(text), mentionedJid: [m.sender, ...conn.parseMention(Function.Format(text))], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
            } else if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            let data = await Function.getFiles(text)
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               ...
               return conn.sendFile(m.from, data.filename, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else {
            if (!!config.msg[text]) text = config.msg[text]
            return conn.sendMessage(chatId, { text, mentions: [m.sender, ...conn.parseMention(text)], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
