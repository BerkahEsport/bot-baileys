 ...
 */
async getFiles(PATH, saveToFile = false) {
    let filename, data;
    if (/^https?:\/\//.test(PATH)) {
        let res = await fetch (this.isUrl(PATH))
    ...
    let size = Buffer.byteLength(data)
    if (data && saveToFile && !filename) (filename = path.join(__dirname, "../tmp/" + +new Date()+ "." + type.ext), await fs.promises.writeFile(filename, data))
    return {
        filename, 
        ...type, 
        data, 
